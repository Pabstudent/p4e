def add_time(start, duration, week=None):
    # Function that returns a specified time based on a start time, duration and optional weekday. 
    # Imports are not allowed. Assumes that the user enters valid strings, since no checks are in place.
    ...
    # Add 12 hours to the temporary end hour in case 'PM' 
    # is given in the start time (so that we can work with a 24h 
    # format instead before reformatting back to 12h)
    if startArr[1] == 'PM':
        tmpEndHour += 12
